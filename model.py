...
from keras.layers import Dense, Input, Flatten, Lambda, Activation, Conv2D, MaxPooling2D, Conv1D, MaxPooling1D, Reshape, Dropout, Average
...
def build_Hierarchical_LSTM_Att():
    ...
    accu_preds = Dense(config.num_accu, activation='softmax', name="accu_preds")(l_att_sent)
    law_preds = Dense(config.num_law, activation='softmax', name="law_preds")(l_att_sent)
    term_preds = Dense(config.num_term, activation='softmax', name="term_preds")(l_att_sent)
...
def build_LSTM_Att():
    ...
    l_lstm = Bidirectional(LSTM(config.hid_size, return_sequences=True))(embedded_sequences)
    l_att = AttLayer(config.attention_dim)(l_lstm)
    accu_preds = Dense(config.num_accu, activation='softmax', name="accu_preds")(l_att)
    law_preds = Dense(config.num_law, activation='softmax', name="law_preds")(l_att)
    term_preds = Dense(config.num_term, activation='softmax', name="term_preds")(l_att)
    model = Model(inputs=[sentence_input], outputs=[accu_preds, law_preds,term_preds])
...
def build_LSTM():
    ...
    accu_preds = Dense(config.num_accu, activation='softmax', name="accu_preds")(l_lstm)
    law_preds = Dense(config.num_law, activation='softmax', name="law_preds")(l_lstm)
    term_preds = Dense(config.num_term, activation='softmax', name="term_preds")(l_lstm)
    model = Model(inputs=[sentence_input], outputs=[accu_preds, law_preds, term_preds])
...
def build_MPBFN_WCA():
    ...
    law_embedding_layer = Embedding(config.num_law,
                                    config.hid_size,
                                    #                                    input_length=config.num_law,
                                    trainable=True,
                                    mask_zero=False)
    
    accu_embedding_layer = Embedding(config.num_accu,
                                     config.hid_size,
                                     #                                     input_length=config.num_accu,
                                     trainable=True,
                                     mask_zero=False)
    
    term_embedding_layer = Embedding(config.num_term,
                                     config.hid_size,
                                     #                                     input_length=config.num_accu,
                                     trainable=True,
                                     mask_zero=False)
    ...
    law_preds1 = Dense(config.num_law, activation='softmax')(feature)
    
    law_input = Input(shape=(config.num_law,),dtype='int32')
    ...
    accu_preds1 = Dense(config.num_accu, activation='softmax')(law_fix)
    
    accu_input = Input(shape=(config.num_accu,),dtype='int32')
    ...
    term_preds1 = Dense(config.num_term, activation ='softmax')(accu_fix)
    term_preds2 = Dense(config.num_term, activation ='softmax')(law_fix)
    
    term_preds3 = multiply([term_preds1, term_preds2])
    term_preds3 = Lambda(normalize, name = "term_preds")(term_preds3)
    
    
    term_input = Input(shape=(config.num_term,),dtype='int32')
    term_embedded_sequences = term_embedding_layer(term_input)
    term_merge = dot([term_preds3,term_embedded_sequences], axes = (1, 1))
    
    
    
    term2law = Dense(config.num_law, activation = 'sigmoid')(term_merge)
    term2accu = Dense(config.num_accu, activation = 'sigmoid')(term_merge)
    
    accu2law = Dense(config.num_law, activation = 'sigmoid')(accu_merge)
